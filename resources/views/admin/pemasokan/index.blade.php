...
<html lang="en">
                  ...
                  <div class="container-fluid">
                      ...
                      <div class="card mb-4">
                        <div class="card-header">
                            ...
                            Pemasok
                            <button href="#add" data-title="Tambah Pemasok" data-mode="add" class="btn btn-sm btn-success float-right trigger" data-toggle="modal" data-target="#form-modal"><i class="fas fa-plus"></i> Tambah</button>
                        </div>

                        @include('admin.templates.feedback')
                        @include('admin.pemasokan.modal')
                        @include('admin.pemasokan.detail')
                        
                        <div class="card-body">
                            <div hidden id="data">{{$pemasok}}</div>
                            <div class="table-responsive">
                                <table class="table table-bordered" id="data-pemasokan" width="100%" cellspacing="0">
                                    <thead>
                                        <tr>
                                            <th>Kode Pemasok</th>
                                            <th>Nama</th>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="btn btn-group">
                                                        <a href="#detail" data-id="{{$r->id}}" class="btn btn-sm btn-dark mr-2 detail" data-toggle="modal" data-target="#detail-pemasokan"><i class="far fa-id-card"></i></a>
                                                        <button href="#edit" data-mode="edit" data-title="Edit Pemasok" data-id="{{$r->id}}" class="btn btn-sm btn-warning mr-2 trigger" data-toggle="modal" data-target="#form-modal"><i class="far fa-edit"></i></button>
                                                        <form action="{{route('pemasokan.index')}}" method="post" id="delete-form">
                                                            @csrf
                                                            @method('delete')
                                                            <button href="#delete" data-mode="delete" data-id="{{$r->id}}" class="btn btn-sm btn-danger delete trigger"><i class="far fa-trash-alt"></i></button>
                                                        </form>
                                                    </div>
                                                </td>
                                            </tr>      
  ...
  <script>
    $(function(){
        ...
        // dataTable
        let table = $('#data-pemasokan').DataTable({
            "paging" : true
        });

        let form = $('form.form-pemasok');
        ...
        $('body').on('click', 'a.detail', function(event){
            event.preventDefault();
            let data = JSON.parse("{{ $pemasok ?? '' }}".replaceAll('&quot;', "\""));
