...
<html lang="en">
                  ...
                  <div class="container-fluid">
                      ...
                      <div class="card mb-4">
                        <div class="card-header">
                            ...
                            Pelanggan
                            <button href="#add" class="btn btn-sm btn-success float-right trigger" data-title="Tambah Pelanggan" data-mode="add" data-toggle="modal" data-target="#form-modal"><i class="fas fa-plus"></i> Tambah</button>
                        </div>

                        @include('admin.templates.feedback')
                        @include('admin.pelanggan.modal')
                        @include('admin.pelanggan.detail')

                        <div class="card-body">
                            <div class="table-responsive">
                                <div hidden id="data">{{$pelanggan}}</div>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="btn btn-group">
                                                        <a href="#detail" data-id="{{$r->id}}" class="btn btn-sm btn-dark mr-2 detail" data-toggle="modal" data-target="#detail-pelanggan"><i class="far fa-id-card"></i></a>
                                                        <button href="#edit" data-id="{{$r->id}}" data-mode="edit" data-title="Edit Pelanggan" class="btn btn-sm btn-warning mr-2 trigger edit" data-toggle="modal" data-target="#form-modal"><i class="far fa-edit"></i></button>
                                                        <form action="{{route('pelanggan.index')}}" method="post" id="delete-form">
                                                            @csrf
                                                            @method('delete')
  ...
  <script>
    $(function(){
        ...
        // dataTable
        let table = $('#data-pelanggan').DataTable({
            "paging" : true
        });

        let form = $('form.form-pelanggan');
        ...
        $('body').on('click', 'a.detail', function(event){
            event.preventDefault();
            let data = JSON.parse("{{ $pelanggan ?? '' }}".replaceAll('&quot;', "\""));
